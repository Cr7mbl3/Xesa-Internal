...
void Chams::OverrideMaterial(bool ignoreZ, bool flat, bool wireframe, bool glass, const Color& rgba) {
	...
	material->SetMaterialVarFlag(MATERIAL_VAR_IGNOREZ, ignoreZ); //TODO: Check out if this crashes sometimes
...
void Chams::PlayerChams(const model_t* model, IMatRenderContext* ctx, const DrawModelState_t& state, const ModelRenderInfo_t& info, matrix3x4_t* matrix)
{
	...
	IClientEntity* clientEntity = g_EntityList->GetClientEntity(info.entity_index);
	...
	C_BasePlayer* player = (C_BasePlayer*) baseEntity;
...
{
	if (!g_LocalPlayer || !g_Engine->IsInGame())
		return;
