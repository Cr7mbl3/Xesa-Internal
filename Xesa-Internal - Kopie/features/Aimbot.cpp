...
void Aimbot::AddHitpoints(std::vector<AimbotData>& vec, C_BasePlayer* basePlayer)
{
	...
	for (int hitbox = HITBOX_HEAD; hitbox < HITBOX_MAX; hitbox++) 
	{
		...
		vec.push_back(AimbotData( right * untransformedBox->m_flRadius, basePlayer ));
		vec.push_back(AimbotData( left * untransformedBox->m_flRadius, basePlayer ));
	}
}
...
void Aimbot::Run(CUserCmd* cmd, bool& bSendPacket)
{
	...
	if (weapon->m_iClip1() == 0) //i dont think this works, did i fix it? //i dont think so
		return;
